package com.emakers.api_biblioteca.services;

import java.time.Instant;
import java.time.LocalDateTime;
import java.time.ZoneOffset;

import org.springframework.beans.factory.annotation.Value;
import org.springframework.stereotype.Service;

import com.auth0.jwt.JWT;
import com.auth0.jwt.algorithms.Algorithm;
import com.auth0.jwt.exceptions.JWTCreationException;
import com.auth0.jwt.exceptions.JWTVerificationException;
import com.emakers.api_biblioteca.models.PessoaModel;

import io.swagger.v3.oas.annotations.tags.Tag;

/**
 * Serviço responsável por gerar e validar tokens JWT para autenticação de usuários.
 */
@Tag(name = "Serviço de Token", description = "Geração e validação de tokens JWT.")
@Service
public class TokenService {

    @Value("${api.security.token.secret:my-secret-key}")
    private String secret;

    /**
     * Gera um token JWT para o usuário fornecido.
     *
     * @param pessoaModel objeto contendo os dados do usuário autenticado
     * @return token JWT gerado
     * @throws RuntimeException se houver erro na geração do token
     */
    public String generateToken(PessoaModel pessoaModel) {
        try {
            Algorithm algorithm = Algorithm.HMAC256(secret);
            return JWT.create()
                      .withIssuer("auth-api")
                      .withSubject(pessoaModel.getEmail())
                      .withExpiresAt(genExpirationDate())
                      .sign(algorithm);
        } catch (JWTCreationException exception) {
            throw new RuntimeException("Erro na geração do token", exception);
        }
    }

    /**
     * Valida o token recebido e retorna o e-mail (subject) associado.
     *
     * @param token JWT a ser validado
     * @return subject (e-mail) se válido, ou string vazia se inválido
     */
    public String validateToken(String token) {
        try {
            Algorithm algorithm = Algorithm.HMAC256(secret);
            return JWT.require(algorithm)
                      .withIssuer("auth-api")
                      .build()
                      .verify(token)
                      .getSubject();
        } catch (JWTVerificationException exception) {
            return "";
        }
    }

    /**
     * Define a data de expiração do token para duas horas a partir do momento atual.
     *
     * @return data e hora de expiração como Instant
     */
    private Instant genExpirationDate() {
        return LocalDateTime.now().plusHours(2).toInstant(ZoneOffset.of("-03:00"));
    }
}
